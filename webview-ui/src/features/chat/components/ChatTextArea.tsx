...
import { CHAT_CONSTANTS } from "./ChatView/constants"
	...
	(
						...
						searchTimeoutRef.current = setTimeout(() => {
							FileServiceClient.searchFiles(
								...
								.then((results: any) => {
									...
									// Auto-dismiss if no results are found and it's not the initial '@'
		...
		const onModeToggle = useCallback(() => {
			void (async () => {
				...
				console.log(`[ChatTextArea] onModeToggle - Toggling to ${convertedProtoMode}, message:`, messageToToggle)
				...
				try {
					...
					if (response.value) {
						// Only clear if the backend acknowledged the toggle
						setInputValue("")
						setSelectedImages([])
						setSelectedFiles([])
					}
				} catch (error) {
					console.error("[ChatTextArea] Failed to toggle mode:", error)
					// Keep input value on error
				} finally {
					// Always try to refocus
					setTimeout(() => {
						textAreaRef.current?.focus()
					}, 100)
				}
		...
		const modelDisplayName = useMemo(() => {
			const { selectedProvider, selectedModelId } = normalizeApiConfiguration(apiConfiguration, mode as Mode)
			const {
			...
			} = getModeSpecificFields(apiConfiguration, mode as Mode)
		...
		return (
			<div className={cn("relative", mode === "plan" ? "bg-grid-plan" : "")}>
				<div
					className={cn(
						"relative flex transition-all duration-300 px-3.5 py-2.5",
						isTextAreaFocused ? "scale-[1.01]" : "scale-100",
					)}
					...
					<div
						className={cn(
							"absolute bottom-2.5 top-2.5 whitespace-pre-wrap break-words rounded-input overflow-hidden transition-all duration-300",
							isTextAreaFocused
								? "left-3.5 right-3.5 glass"
								: "left-3.5 right-3.5 border border-input-border bg-input-background",
						)}
						...
						style={{
							...
							outline:
								isDraggingOver && !showUnsupportedFileError // Only show drag outline if not showing error
									...
									: isTextAreaFocused
										? `1px solid ${mode === "plan" ? "var(--color-glow-plan)" : "var(--color-glow-act)"}`
										: "none",
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<div className="relative flex-1 min-w-0 h-5">
						...
						<div className="absolute top-0 left-0 right-0 ease-in-out w-full h-5 z-10 flex items-center gap-1 flex-1 min-w-0">
							<Tooltip>
							...
							<div className="relative flex flex-1 min-w-0">
								<div className="inline-flex min-w-0 max-w-full">
									<a
										className={cn(
											"px-0 h-5 w-full min-w-0 flex items-center text-[10px] outline-none select-none",
